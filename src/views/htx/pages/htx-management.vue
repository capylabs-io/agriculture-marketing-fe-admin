...
export default {
  ...
  methods: {
    ...
    onOpenClicked(code) {
      const link = process.env.VUE_APP_USER_PAGE + "hop-tac-xa/" + code;
      window.open(link);
    },
    onEditClicked(item) {
      this.htxStore.htx = item;
      this.htxStore.htx.cooperativeCategory = item.cooperativeCategory.id;
    ...
    onDeleteClicked(htxId) {
      this.$dialog.confirm({
        title: "Xác nhận xóa Hợp tác xã",
        topContent: "Bạn có chắc bạn muốn xóa Hợp tác xã này không?",
        midContent:
