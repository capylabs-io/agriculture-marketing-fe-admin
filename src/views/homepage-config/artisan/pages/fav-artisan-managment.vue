...
<template>
  <div>
    ...
    <div class="d-flex align-center justify-space-between mt-6">
      ...
      >
        ...
        <v-tooltip bottom>
          ...
          <div>
            Số lượng Nghệ nhân {{ favArtisanStore.favArtisanCodes.length }}/4
          </div>
        </v-tooltip>
      ...
      <div class="d-flex">
        ...
        <v-btn
          v-if="favArtisanStore.favArtisanCodes.length < 4"
          class="white-bg neutral20-border text-none btn-text border-radius-8 py-5"
          elevation="0"
          outlined
          @click="favArtisanStore.favArtisanCreateDialog = true"
        >
          <v-icon small>mdi-plus</v-icon>
    ...
    <div class="border-radius-12 neutral20-border overflow-hidden mt-3">
      <v-data-table
        :headers="headers"
        :items="favArtisanStore.favArtisans ? favArtisanStore.favArtisans : ''"
        hide-default-footer
      >
        <template v-slot:body="props">
          <draggable
            v-model="favArtisanStore.favArtisans"
            animation="150"
            :move="onMoveCallback"
...
export default {
  ...
  components: {
    CreateDialog: () => import("../dialogs/create-favArtisan-dialog.vue"),
    dataTableRowHandler: () =>
  ...
  async created() {
    await this.favArtisanStore.fetchfavArtisanCodes();
    await this.favArtisanStore.fetchfavArtisans();
  },
  methods: {
    getImageUrl(url) {
    ...
    onDropCallback(evt, originalEvent) {
      ...
      console.log("favArtisans", this.favArtisanStore.favArtisans);
