...
<template>
  <div>
    ...
    <div class="d-flex align-center justify-space-between mt-6">
      ...
      <div class="d-flex">
        ...
        <v-btn
          ...
          elevation="0"
          outlined
          @click="favRegionStore.favRegionCreateDialog = true"
        >
          <v-icon small>mdi-plus</v-icon>
    ...
    <div class="border-radius-12 neutral20-border overflow-hidden mt-3">
      <v-data-table
        :headers="headers"
        :items="favRegionStore.favRegions ? favRegionStore.favRegions : ''"
        hide-default-footer
      >
        <template v-slot:body="props">
          <draggable
            v-model="favRegionStore.favRegions"
            animation="150"
            :move="onMoveCallback"
          ...
          >
            ...
            <tr
              v-if="!favRegionStore.favRegions.length > 0"
              class="text-center"
            >
...
export default {
  ...
  components: {
    CreateDialog: () => import("../dialogs/create-favRegion-dialog.vue"),
    dataTableRowHandler: () =>
  ...
  async created() {
    await this.favRegionStore.fetchfavRegionCodes();
    await this.favRegionStore.fetchfavRegions();
  },
  methods: {
    getImageUrl(url) {
      ...
      this.$dialog.confirm({
        ...
        midContent:
          ...
          this.favRegionStore.favRegionCodes =
            this.favRegionStore.favRegionCodes.filter((p) => p !== productCode);
          await this.favRegionStore.updatefavRegions();
        },
      });
    },
    async updateOrderList() {
      this.favRegionStore.favRegionCodes = this.favRegionStore.favRegions.map(
        (p) => p.code
      );
      await this.favRegionStore.updatefavRegions();
    ...
    onDropCallback(evt, originalEvent) {
      ...
      console.log("favRegions", this.favRegionStore.favRegions);
